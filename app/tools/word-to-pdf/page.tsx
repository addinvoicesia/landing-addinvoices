...
import { ChevronRight, FileText, Download, ShieldCheck, AlertTriangle, FileUp, Loader2 } from "lucide-react";
...
export default function WordToPDFPage() {
  ...
  const [scriptsLoaded, setScriptsLoaded] = useState({ docx: false, html2canvas: false, jspdf: false });
  ...
  const renderPreview = async (file: File) => {
    ...
    try {
      await (window as any).docx.renderAsync(arrayBuffer, previewContainerRef.current);
    } catch (e) {
      console.error("Preview error", e);
      setError("We couldn't read this file correctly. Complex formatting may vary.");
    }
  };

  const convertToPDF = async () => {
    if (!file || !scriptsLoaded.html2canvas || !scriptsLoaded.jspdf || !previewContainerRef.current) return;
    ...
    try {
      ...
      setError("Error during conversion. Complex formatting might be the cause.");
      setIsProcessing(false);
    }
  };

  const allScriptsLoaded = scriptsLoaded.docx && scriptsLoaded.html2canvas && scriptsLoaded.jspdf;
    ...
    <div className="min-h-screen w-full relative bg-ad-main font-sans overflow-x-hidden">
      ...
      <main className="relative z-10 pt-28 pb-20 px-4 md:px-6 max-w-6xl mx-auto">
        ...
        <div className="text-center mb-16 px-4">
          <h1 className="text-4xl md:text-6xl font-black text-white mb-6">Word to PDF Converter</h1>
          <p className="text-ad-secondary text-lg max-w-2xl mx-auto leading-relaxed">
            Convert your Word documents to PDF instantly. Free, secure, and processed entirely in your browser.
          </p>
        </div>
             ...
             >
                <div className="space-y-6">
                   ...
                   <div className="space-y-2">
                      <h3 className="text-xl font-bold text-white uppercase tracking-tight">Drop your Word file here</h3>
                      <p className="text-gray-500 text-sm">.docx and .doc files supported (Max 10MB)</p>
                   </div>
             ...
             <div className="space-y-6">
                <div className="bg-[#111111] border border-white/10 rounded-2xl p-6 flex items-center justify-between">
                   <div className="flex items-center gap-4">
                      ...
                      <div className="space-y-0.5">
                         <div className="text-white font-bold text-sm truncate max-w-[200px] md:max-w-md">{file.name}</div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
                   ...
                   <div className="lg:col-span-8 bg-white rounded-xl shadow-2xl p-8 min-h-[500px] max-h-[800px] overflow-y-auto custom-scrollbar-light relative">
                      <div ref={previewContainerRef} className="docx-preview h-full" />
                      {isProcessing && (
                        <div className="absolute inset-0 bg-white/80 backdrop-blur-sm flex flex-col items-center justify-center p-10 text-center">
                           ...
                           <h4 className="text-xl font-bold text-black mb-2">Converting document...</h4>
                           ...
                           <p className="mt-4 text-[10px] text-gray-400 font-bold uppercase tracking-widest leading-loose">
                              Optimizing fonts and layouts for PDF export. <br/> Almost there.
                           </p>
                        </div>
                      )}
                   </div>
                   ...
                   <div className="lg:col-span-4 space-y-6">
                      <div className="bg-blue-600/5 border border-blue-600/10 rounded-2xl p-6 space-y-4">
                         <div className="text-[10px] font-bold text-blue-400 uppercase tracking-widest">Ready to go</div>
                         <h3 className="text-white font-bold leading-tight">Your document is loaded and ready for conversion.</h3>
                         <button 
                         ...
                         <p className="text-[10px] text-gray-600 text-center leading-relaxed italic">
                           Note: Complex formatting like heavy tables or special fonts may vary slightly from the original.
                         </p>
                      </div>
                      ...
                      <div className="p-6 bg-amber-500/5 border border-amber-500/10 rounded-2xl flex gap-4">
                         ...
                         <p className="text-[11px] text-amber-500/80 leading-relaxed font-medium">
                            If your document has complex layers, try opening it in Google Docs and exporting to PDF for perfect results.
                         </p>
                      </div>
                   </div>
                </div>
        ...
        <div className="mt-20 max-w-4xl mx-auto">
           <div className="bg-[#111111] border border-white/10 rounded-2xl p-10 flex flex-col md:flex-row items-center gap-10">
              ...
              <div className="space-y-2 text-center md:text-left">
                 <h3 className="text-xl font-bold text-white">Your files never leave your device</h3>
                 <p className="text-ad-secondary text-sm leading-relaxed">
                   Unlike most online converters, we process your files entirely in your browser using local resources. No documents are uploaded to any server. Your privacy is guaranteed by design.
                 </p>
              </div>
           </div>
        </div>
        ...
        <div className="mt-32 bg-[#111111] rounded-2xl p-10 md:p-16 border border-white/10 text-center relative overflow-hidden shadow-2xl">
          ...
          <p className="text-ad-secondary text-lg mb-10 max-w-2xl mx-auto leading-relaxed">
            AddInvoices lets you create, customize, and send invoices as PDF directly to your clients — no Word or manual conversion needed.
          </p>
          <a
