...
import { 
  FileText, Mic, Calculator, Layout, FileSignature, 
  ArrowRight, Sparkles, Hash, QrCode, Mail, 
  Image as ImageIcon, FileOutput, Lightbulb, Receipt
} from "lucide-react";
...
const groups = [
  {
    ...
    tools: [
      ...
      },
      {
        ...
        description: "Create a professional numbering system for your business.",
        ...
        <section className="text-center mb-24 relative">
           ...
           >
              ...
              <h1 className="text-5xl md:text-7xl font-black text-white mb-6 tracking-tight">
                 Free tools for your <br/> <span className="text-blue-500">growing business.</span>
              </h1>
              <p className="text-ad-secondary text-lg max-w-2xl mx-auto leading-relaxed">
                 Professional, high-quality tools to help you manage your invoicing, documents, and business admin — 100% free and secure.
              </p>
           </motion.div>
        </section>
           ...
           {groups.map((group, gIdx) => (
             <section key={group.title} className="space-y-10">
                <div className="flex items-center gap-6">
                   <h2 className="text-2xl font-black text-white whitespace-nowrap tracking-tight">{group.title}</h2>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                   {group.tools.map((tool, tIdx) => (
                     ...
                     >
                       ...
                       >
                          <div className={`w-12 h-12 ${tool.color} rounded-2xl flex items-center justify-center mb-6 group-hover:scale-110 transition-transform shadow-lg`}>
                             <tool.icon className="w-6 h-6 text-white" />
                          </div>
                          
                          <div className="flex-1 space-y-3">
                             <h3 className="text-white font-bold text-lg leading-tight group-hover:text-blue-400 transition-colors uppercase tracking-tight">{tool.title}</h3>
                             <p className="text-ad-secondary text-xs leading-relaxed opacity-80 group-hover:opacity-100 transition-opacity">{tool.description}</p>
                          </div>
                          ...
                          <div className="mt-8 flex items-center gap-2 text-blue-500 text-[10px] font-black uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-opacity">
                             Open Tool <ArrowRight className="w-3.5 h-3.5 translate-x-0 group-hover:translate-x-1 transition-transform" />
                          </div>

                          {/* Subtle background glow on hover */}
                          <div className={`absolute top-0 right-0 w-24 h-24 ${tool.color} opacity-0 group-hover:opacity-10 blur-3xl rounded-full transition-opacity`} />
                       </Link>
                     </motion.div>
                   ))}
                </div>
           ...
           <div className="bg-[#111111] rounded-[40px] p-12 md:p-24 border border-white/10 text-center relative overflow-hidden shadow-2xl">
              ...
              <div className="relative z-10 max-w-3xl mx-auto space-y-10">
                 <h2 className="text-4xl md:text-6xl font-black text-white leading-tight">
                    Tired of manual docs? <br/> <span className="text-blue-500">Upgrade to AddInvoices.</span>
                 </h2>
                 <p className="text-ad-secondary text-xl font-medium max-w-xl mx-auto leading-relaxed">
                    Join thousands of freelancers and agencies who save hours every week by automating their invoicing and collections.
                 </p>
