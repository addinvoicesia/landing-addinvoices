...
export default function ProfitMarginCalculatorPage() {
  ...
  const recommendedPrice = desiredMargin < 100 ? cost / (1 - desiredMargin / 100) : 0;
    ...
    <div className="min-h-screen w-full relative bg-ad-main font-sans overflow-x-hidden">
      ...
      <main className="relative z-10 pt-28 pb-20 px-4 md:px-6 max-w-4xl mx-auto">
        ...
        <nav className="flex items-center space-x-2 text-xs md:text-sm text-ad-secondary mb-8">
          ...
          <ChevronRight className="w-3 h-3" />
          <span className="text-white font-medium">Profit Margin Calculator</span>
        </nav>
        ...
        <div className="flex items-center justify-center p-1 bg-white/5 border border-white/10 rounded-2xl w-fit mx-auto mb-10 [color-scheme:dark]">
          <button
            ...
            className={`px-6 py-3 rounded-xl text-sm font-bold transition-all ${
              mode === "margin" ? "bg-blue-600 text-white shadow-lg" : "text-gray-400 hover:text-white"
            }`}
          >
            Calculate my margin
          </button>
          <button
            ...
            className={`px-6 py-3 rounded-xl text-sm font-bold transition-all ${
              mode === "price" ? "bg-blue-600 text-white shadow-lg" : "text-gray-400 hover:text-white"
            }`}
          >
            Find my sell price
          </button>
            ...
            {mode === "margin" ? (
              <>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <label className="text-xs font-medium text-gray-500 uppercase tracking-wider">Cost Price ($)</label>
                    <input 
                      type="number" 
                      value={cost} 
                ...
                <div className="pt-8 border-t border-white/10 space-y-6">
                  <div className="grid grid-cols-2 gap-4">
                     <div className="p-4 bg-white/[0.02] border border-white/10 rounded-2xl">
                        <div className="text-[10px] text-gray-500 uppercase font-bold mb-1">Gross Profit</div>
                        <div className="text-2xl font-bold text-white">${grossProfit.toLocaleString()}</div>
                     </div>
                     <div className="p-4 bg-white/[0.02] border border-white/10 rounded-2xl">
                        <div className="text-[10px] text-gray-500 uppercase font-bold mb-1">Markup</div>
                        <div className="text-2xl font-bold text-white">{markup.toFixed(1)}%</div>
                     </div>
                  </div>

                  <div className="text-center p-6 bg-blue-600/5 border border-blue-500/10 rounded-2xl">
                     <div className="text-xs text-blue-400 uppercase font-bold mb-2 tracking-widest">Profit Margin</div>
                     <div className="text-5xl font-black text-blue-500">{margin.toFixed(1)}%</div>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="w-full h-3 bg-white/5 rounded-full overflow-hidden flex [color-scheme:dark]">
                       ...
                       />
                       <div 
                        style={{ width: `${(grossProfit / sellingPrice) * 100}%` }} 
              ...
              <>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-2">
                    <label className="text-xs font-medium text-gray-500 uppercase tracking-wider">Desired Margin (%)</label>
                    <input 
                      type="number" 
                      value={desiredMargin} 
                ...
                <div className="pt-8 border-t border-white/10 space-y-6">
                  <div className="text-center p-8 bg-blue-600/5 border border-blue-500/10 rounded-2xl">
                     <div className="text-xs text-blue-400 uppercase font-bold mb-3 tracking-widest">Recommended Price</div>
                     <div className="text-5xl font-black text-blue-500">
                        ${recommendedPrice.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                     </div>
                  </div>

                  <div className="grid grid-cols-2 gap-4">
                     <div className="p-5 bg-white/[0.02] border border-white/10 rounded-2xl">
                        <div className="text-[10px] text-gray-500 uppercase font-bold mb-1">Your Profit</div>
                        <div className="text-2xl font-bold text-white">${priceProfit.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div>
                     </div>
                     <div className="p-5 bg-white/[0.02] border border-white/10 rounded-2xl">
                        <div className="text-[10px] text-gray-500 uppercase font-bold mb-1">Markup</div>
                        <div className="text-2xl font-bold text-white">{priceMarkup.toFixed(1)}%</div>
                     </div>
                  </div>
                </div>
              </>
           ...
           <div className="space-y-4">
              ...
              <div className="p-4 bg-white/5 border border-white/10 rounded-2xl text-xs space-y-3 [color-scheme:dark]">
                 <div className="flex justify-between">
                    ...
                    <span className="text-white font-medium">(Profit / Revenue) × 100</span>
                 </div>
                 <div className="flex justify-between">
                    <span className="text-gray-400">Markup:</span>
                    <span className="text-white font-medium">(Profit / Cost) × 100</span>
                 </div>
              </div>
           </div>
           <div className="space-y-4">
        ...
        <div className="bg-[#111111] rounded-2xl p-10 md:p-16 border border-white/10 text-center relative overflow-hidden shadow-2xl">
          ...
          <p className="text-ad-secondary text-lg mb-10 max-w-2xl mx-auto leading-relaxed">
            Track your income, expenses, and profitability automatically with AddInvoices.
          </p>
          <a
