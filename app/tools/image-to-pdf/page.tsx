...
import { ChevronRight, Image as ImageIcon, Download, Trash2, GripVertical, FileUp, Loader2, Info } from "lucide-react";
...
export default function ImageToPDFPage() {
  ...
  const [scriptsLoaded, setScriptsLoaded] = useState({ jspdf: false, sortable: false });
  ...
  const convertToPDF = async () => {
    ...
    const doc = new jsPDF(orientation, "mm", pageSize === "auto" ? "a4" : pageSize);
    ...
    for (let i = 0; i < images.length; i++) {
        ...
        doc.addImage(imgData, "JPEG", margin, margin, effectiveWidth, effectiveHeight, undefined, "FAST");
    }

    // Attribution (Level 2)
    drawSmallAttribution(doc);
    
    doc.save(`Images_to_PDF_AddInvoices_${new Date().toISOString().split('T')[0]}.pdf`);
    ...
    <div className="min-h-screen w-full relative bg-ad-main font-sans overflow-x-hidden">
      ...
      <main className="relative z-10 pt-28 pb-20 px-4 md:px-6 max-w-6xl mx-auto">
        ...
        <div className="text-center mb-16">
          <h1 className="text-4xl md:text-6xl font-black text-white mb-6">Image to PDF Converter</h1>
          <p className="text-ad-secondary text-lg max-w-2xl mx-auto leading-relaxed">
            Convert JPG, PNG or WEBP images to PDF. Combine multiple images into one document.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
              ...
              >
                  <div className="flex flex-col items-center gap-6">
                     ...
                     <div className="space-y-1">
                        <h4 className="text-xl font-bold text-white uppercase tracking-tight">Drop images here</h4>
                        <p className="text-gray-500 text-sm">JPG, PNG, WEBP · Max 20MB total</p>
                     </div>
                     <button className="px-8 py-3 bg-white/5 border border-white/10 rounded-xl text-white font-bold [color-scheme:dark]">
                        Choose images
                     </button>
                     <input type="file" id="image-upload" className="hidden" multiple accept="image/*" onChange={handleFiles} />
                  </div>
              </div>
              ...
              {images.length > 0 && (
                <div className="space-y-4">
                   <div className="flex items-center justify-between px-2">
                      <h4 className="text-xs font-bold text-gray-400 uppercase tracking-widest">Images reorderable ({images.length})</h4>
                      <p className="text-[10px] text-gray-600 italic">Drag to reorder elements</p>
                   </div>
                   <div ref={listRef} className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                      {images.map((img, index) => (
                        <div key={img.id} className="relative group aspect-[3/4] bg-[#141414] border border-white/10 rounded-2xl overflow-hidden shadow-xl">
                           <img src={img.preview} className="w-full h-full object-cover opacity-60 group-hover:opacity-100 transition-opacity" />
           ...
           <div className="lg:col-span-4 sticky top-28 space-y-6">
                 ...
                 <div className="relative z-10 space-y-6">
                    ...
                    <div className="space-y-4">
                       <div className="space-y-2">
                          <label className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">Page Size</label>
                          <div className="grid grid-cols-2 gap-2">
                             {(["auto", "a4", "letter"] as const).map(s => (
                               <button 
                                key={s} onClick={() => setPageSize(s)}
                                className={`py-2 text-[10px] font-bold rounded-lg border transition-all ${
                                  pageSize === s ? "bg-blue-600 border-blue-500 text-white" : "bg-white/5 border-white/10 text-gray-500"
                                }`}
                               >
                                  {s.toUpperCase()}
                               </button>
                       ...
                       <div className="space-y-2">
                          <label className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">Orientation</label>
                          <div className="flex gap-2">
                             <button 
                              onClick={() => setOrientation("p")}
                              className={`flex-1 py-2 text-[10px] font-bold rounded-lg border transition-all ${
                                orientation === "p" ? "bg-blue-600 border-blue-500 text-white" : "bg-white/5 border-white/10 text-gray-500"
                              }`}
                             >
                                Portrait
                             </button>
                             <button 
                              ...
                              className={`flex-1 py-2 text-[10px] font-bold rounded-lg border transition-all ${
                                orientation === "l" ? "bg-blue-600 border-blue-500 text-white" : "bg-white/5 border-white/10 text-gray-500"
                              }`}
                             >
                                Landscape
                             </button>
                    ...
                    <div className="pt-6 border-t border-white/10">
                       ...
                       <p className="mt-3 text-[10px] text-gray-600 text-center leading-relaxed">
                          Your images are processed 100% locally. <br/> Never leaving your browser.
                       </p>
                    </div>
                 </div>
              </div>
              ...
              <div className="bg-blue-600/5 border border-blue-600/10 rounded-2xl p-6 flex gap-4">
                 ...
                 <div className="space-y-1">
                    <h5 className="text-[10px] font-bold text-white uppercase tracking-widest">Did you know?</h5>
                    <p className="text-[11px] text-white/50 leading-relaxed font-medium">
                       Combine your scanned receipts into a single PDF to easily track them as expenses in AddInvoices.
                    </p>
                 </div>
              </div>
           </div>
        ...
        <div className="mt-40 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
           <div className="space-y-4">
              ...
              <p className="text-ad-secondary text-xs leading-loose">
                 Turn photos of your physical receipts into a clean PDF document for bookkeeping or tax season compliance.
              </p>
           </div>
           <div className="space-y-4">
              <h4 className="text-white font-bold">Combine photos</h4>
              <p className="text-ad-secondary text-xs leading-loose">
                 Merge multiple progress shots, screenshots, or design concepts into a single professional PDF presentation.
              </p>
           </div>
           <div className="space-y-4">
              <h4 className="text-white font-bold">Convert screenshots</h4>
              <p className="text-ad-secondary text-xs leading-loose">
                 Easily document bug reports or UI examples by turning multiple screenshots into a portable PDF file.
              </p>
           </div>
           <div className="space-y-4">
              <h4 className="text-white font-bold">Create a PDF portfolio</h4>
              <p className="text-ad-secondary text-xs leading-loose">
                 Assemble your best visual works into a single high-quality PDF to share with potential clients and partners.
              </p>
           </div>
        </div>
        ...
        <div className="mt-32 bg-[#111111] rounded-2xl p-10 md:p-16 border border-white/10 text-center relative overflow-hidden shadow-2xl">
          ...
          <p className="text-ad-secondary text-lg mb-10 max-w-2xl mx-auto leading-relaxed">
            AddInvoices lets you log and categorize expenses, attach receipts, and prepare for tax season automatically. No more shoeboxes of paper.
          </p>
          <a
