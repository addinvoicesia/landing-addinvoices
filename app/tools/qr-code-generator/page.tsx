...
import { ChevronRight, Globe, AlignLeft, Mail, Phone, Download, Copy, Check } from "lucide-react";
...
export default function QRCodeGeneratorPage() {
  ...
  const getQRValue = () => {
    switch (type) {
      case "url": return url || "https://addinvoicesai.com";
      case "text": return text || "AddInvoices";
      case "email": return `mailto:${email}?subject=${encodeURIComponent(emailSubject)}`;
      case "phone": return `tel:${phoneNumber}`;
      default: return "https://addinvoicesai.com";
    }
  };
  ...
  const generateQR = () => {
    ...
    try {
      new (window as any).QRCode(qrRef.current, {
        ...
        colorDark: fgColor,
        colorLight: bgColor,
        correctLevel: (window as any).QRCode.CorrectLevel[errorCorrection === "L" ? "L" : errorCorrection === "H" ? "H" : "M"],
      });
  ...
  useEffect(() => {
    generateQR();
  }, [type, url, text, email, emailSubject, phoneNumber, size, errorCorrection, fgColor, bgColor, isQRCodeLoaded]);
  ...
  const downloadPNG = () => {
    ...
    ctx.fillText("Generated with AddInvoices · addinvoicesai.com", finalCanvas.width / 2, canvas.height + 18);
    
    const link = document.createElement("a");
    const filename = `QR_${(getQRValue().substring(0, 20)).replace(/[^a-z0-9]/gi, "_")}_AddInvoices.png`;
    ...
    <div className="min-h-screen w-full relative bg-ad-main font-sans overflow-x-hidden">
      ...
      <main className="relative z-10 pt-28 pb-20 px-4 md:px-6 max-w-6xl mx-auto">
        ...
        <div className="text-center mb-16">
          <h1 className="text-4xl md:text-6xl font-black text-white mb-6 tracking-tight">Free QR Code Generator</h1>
          <p className="text-ad-secondary text-lg max-w-2xl mx-auto">
            Create a QR code for any URL, text, or payment link. Free, instant, no account needed.
          </p>
        </div>
           ...
           <div className="lg:col-span-7 bg-[#111111] border border-white/10 rounded-2xl p-8 space-y-8 shadow-2xl">
                 ...
                 {[
                   ...
                   <button
                    ...
                    className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg text-xs font-bold transition-all ${
                      type === t.id ? "bg-blue-600 text-white shadow-lg" : "text-gray-500 hover:text-gray-300"
                    }`}
                   >
              ...
              <div className="space-y-6 min-h-[140px]">
                 {type === "url" && (
                   <div className="space-y-2">
                      <label className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">URL / Link</label>
                      <input 
                        type="url" 
                        value={url} 
                 ...
                 {type === "text" && (
                   <div className="space-y-2">
                      <label className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">Plain Text</label>
                      <textarea 
                        value={text} 
                 ...
                 {type === "email" && (
                   <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div className="space-y-2">
                        <label className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">Subject (Optional)</label>
                        <input 
                          type="text" 
                          value={emailSubject} 
              ...
              <div className="pt-8 border-t border-white/10 grid grid-cols-1 md:grid-cols-2 gap-8">
                 <div className="space-y-6">
                          ...
                          {[
                            ...
                            <button
                              ...
                              className={`flex-1 py-2 text-[10px] font-bold rounded-lg border transition-all ${
                                size === s.val ? "bg-blue-600 border-blue-500 text-white" : "bg-white/5 border-white/10 text-gray-500"
                              }`}
                            >
                               {s.label}
                            </button>
                    ...
                    <div className="space-y-3">
                       <label className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">Error Correction</label>
                       <div className="flex gap-4">
                          {["L", "M", "H"].map(l => (
                            <button
                              key={l}
                              ...
                              className={`flex-1 py-2 text-[10px] font-bold rounded-lg border transition-all ${
                                errorCorrection === l ? "bg-blue-600 border-blue-500 text-white" : "bg-white/5 border-white/10 text-gray-500"
                              }`}
                            >
                 ...
                 <div className="space-y-6">
                    <div className="space-y-3">
                       <label className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">Foreground Color</label>
                       <div className="flex items-center gap-4">
                          ...
                          />
                          <span className="text-white text-sm font-mono uppercase">{fgColor}</span>
                       </div>
                    </div>
                    <div className="space-y-3">
                       ...
                       <div className="flex items-center gap-4">
                          <input 
                            ...
                            className="w-12 h-12 rounded-lg bg-transparent border border-white/10 cursor-pointer"
                          />
                          <span className="text-white text-sm font-mono uppercase">{bgColor}</span>
                       </div>
                    </div>
                 </div>
              </div>
           ...
           <div className="lg:col-span-5 sticky top-28 space-y-8">
              <div className="bg-white rounded-2xl p-10 shadow-2xl relative overflow-hidden flex flex-col items-center">
                 <div className="relative mb-6">
                    <div ref={qrRef} className="p-4 bg-white border border-gray-100 rounded-2xl shadow-sm" />
                 </div>
                 ...
                 <div className="mt-8 p-4 bg-blue-50 rounded-2xl border border-blue-100 flex items-start gap-4">
                    ...
                    <div className="space-y-1">
                       <div className="text-[11px] font-bold text-blue-900 leading-tight">
                          Pro tip: Add this QR to your invoices so clients can pay instantly by scanning it.
                       </div>
                       <a href="https://app.addinvoicesai.com" className="text-[10px] font-bold text-blue-600 hover:underline">
        ...
        <div className="mt-32 max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-12">
           <div className="space-y-4">
              <h3 className="text-xl font-bold text-white tracking-tight">QR code for payment links</h3>
              <p className="text-ad-secondary text-sm leading-relaxed">
                Paste your Stripe, PayPal, or Venmo link to generate a QR code. Place it on your physical invoices or store signage so clients can scan and pay you in seconds.
              </p>
           </div>
           <div className="space-y-4">
              <h3 className="text-xl font-bold text-white tracking-tight">QR code for your website or portfolio</h3>
              <p className="text-ad-secondary text-sm leading-relaxed">
                Send potential clients directly to your online portfolio or "Book a Call" page. Perfect for business cards, flyers, and event presentations.
              </p>
           </div>
           <div className="space-y-4">
              <h3 className="text-xl font-bold text-white tracking-tight">QR code for business cards</h3>
              <p className="text-ad-secondary text-sm leading-relaxed">
                Include a QR code on the back of your business card that either points to your digital contact profile (vCard) or your primary business landing page.
              </p>
           </div>
           <div className="space-y-4">
              <h3 className="text-xl font-bold text-white tracking-tight">QR code for contact information</h3>
              <p className="text-ad-secondary text-sm leading-relaxed">
                Generate a QR code with your phone number or email address pre-filled, making it effortless for new clients to save your details and reach out.
              </p>
           </div>
        </div>
        ...
        <div className="mt-32 bg-[#111111] rounded-2xl p-10 md:p-16 border border-white/10 text-center relative overflow-hidden shadow-2xl">
          ...
          <p className="text-ad-secondary text-lg mb-10 max-w-2xl mx-auto">
            With AddInvoices, your clients can pay with one scan. We generate unique tracking links for every bill.
          </p>
          <a
