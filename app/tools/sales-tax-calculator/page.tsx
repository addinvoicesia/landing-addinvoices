      ...
      <main className="relative z-10 pt-28 pb-20 px-4 md:px-6 max-w-4xl mx-auto">
        ...
        <div className="bg-[#141414] border border-white/10 rounded-2xl p-8 md:p-12 shadow-2xl mb-16 max-w-xl mx-auto">
          <div className="space-y-6">
            <div className="space-y-2">
              <label className="text-xs font-medium text-gray-500 uppercase tracking-wider">Subtotal Amount ($)</label>
              <input 
                type="number" 
                value={subtotal} 
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                ...
                >
                  {Object.keys(stateTaxRates).sort().map(code => (
                    <option key={code} value={code} className="bg-[#111111]">{code}</option>
                  ))}
                </select>
              </div>
              <div className="space-y-2">
                <label className="text-xs font-medium text-gray-500 uppercase tracking-wider">Local Tax (%)</label>
                <input 
                  type="number" 
                  value={localTax} 
            ...
            <div className="pt-6 border-t border-white/10 space-y-4">
              ...
              <div className="flex justify-between text-gray-400">
                ...
                <span>${stateTaxAmount.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
              </div>
              {localTax > 0 && (
                <div className="flex justify-between text-gray-400">
                  <span>Local Tax ({localTax}%):</span>
                  <span>${localTaxAmount.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                </div>
              )}
        ...
        <div className="mb-20">
          ...
          >
            ...
            <Calculator className={`w-4 h-4 transition-transform ${showAllRates ? "rotate-180" : ""}`} />
          </button>
            ...
            <div className="overflow-hidden rounded-2xl border border-white/10 bg-white/5 [color-scheme:dark]">
              <table className="w-full text-left text-sm">
                ...
                <tbody className="divide-y divide-white/5">
                  {Object.entries(stateTaxRates).sort().map(([code, rate]) => (
                    <tr key={code} className="hover:bg-white/[0.02] transition-colors">
                      <td className="px-6 py-4 text-white font-medium">{code}</td>
                      <td className="px-6 py-4 text-gray-300">{rate.toFixed(2)}%</td>
                      <td className="px-6 py-4 text-gray-500 text-xs">
                        {rate === 0 ? "No state sales tax" : "Standard state rate"}
                      </td>
                    </tr>
                  ))}
                </tbody>
        ...
        <div className="bg-[#111111] rounded-2xl p-10 md:p-16 border border-white/10 text-center relative overflow-hidden shadow-2xl">
          ...
          <p className="text-ad-secondary text-lg mb-10 max-w-2xl mx-auto leading-relaxed relative z-10">
            AddInvoices automatically calculates and applies sales tax when you create invoices — no manual calculation needed.
          </p>
          <a
