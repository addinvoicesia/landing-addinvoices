...
export default function FreelanceRateCalculatorPage() {
  ...
  const totalAnnualBillableHours = workingWeeksPerYear * billableHoursPerWeek * (1 - nonBillablePercent / 100);

  const minimumRate = totalAnnualBillableHours > 0 ? totalRevenueNeeded / totalAnnualBillableHours : 0;
  const recommendedRate = minimumRate / (1 - 0.20); // 20% margin

  const addExpense = () => {
    setExpenses([...expenses, { id: Math.random().toString(36).substr(2, 9), label: "Other", amount: 0 }]);
  };
  ...
  const getInsight = () => {
    if (recommendedRate < 25) return "Most freelancers in your field charge $40-80/hr. You may be undercharging.";
    if (recommendedRate < 60) return "You're in the typical range for independent professionals. Consider raising rates as you gain experience.";
    ...
    <div className="min-h-screen w-full relative bg-ad-main font-sans overflow-x-hidden">
      ...
      <main className="relative z-10 pt-28 pb-20 px-4 md:px-6 max-w-4xl mx-auto">
        ...
        <nav className="flex items-center space-x-2 text-xs md:text-sm text-ad-secondary mb-8">
          ...
          <ChevronRight className="w-3 h-3" />
          <span className="text-white font-medium">Freelance Rate Calculator</span>
        </nav>
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-ad-secondary text-lg max-w-2xl mx-auto">
            Find out exactly what to charge per hour. Based on your real expenses and income goals.
          </p>
        </div>
          ...
          <div className="lg:col-span-12 space-y-12 max-w-2xl mx-auto w-full">
            ...
            <div className="bg-[#141414] border border-white/10 rounded-2xl p-8 space-y-6 shadow-2xl">
               ...
               <div className="space-y-4">
                  {expenses.map((exp) => (
                    <div key={exp.id} className="flex items-center gap-4">
                       ...
                       <button onClick={() => removeExpense(exp.id)} className="p-2 text-gray-600 hover:text-red-500 transition-colors">
               ...
               <div className="pt-6 border-t border-white/10 flex justify-between items-center">
                  <span className="text-gray-400 font-medium">Total monthly expenses:</span>
                  <span className="text-xl font-bold text-white">${totalMonthlyExpenses.toLocaleString()}</span>
               </div>
            </div>
            ...
            <div className="bg-[#141414] border border-white/10 rounded-2xl p-8 space-y-6 shadow-2xl">
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="space-y-2">
                    <label className="text-xs font-medium text-gray-500 uppercase tracking-wider">Desired Annual Salary ($)</label>
                    <input 
                      type="number" 
                      value={desiredSalary} 
                  ...
                  <div className="space-y-2">
                    <label className="text-xs font-medium text-gray-500 uppercase tracking-wider">Vacation Time (Weeks/Year)</label>
                    <input 
                      type="number" 
                      value={vacationWeeks} 
                  ...
                  <div className="space-y-2">
                    <label className="text-xs font-medium text-gray-500 uppercase tracking-wider">Billable Hours / Week</label>
                    <input 
                      type="number" 
                  ...
                  <div className="space-y-2">
                    <label className="text-xs font-medium text-gray-500 uppercase tracking-wider">Non-billable Time (%)</label>
                    <input 
                      type="number" 
                      value={nonBillablePercent} 
                      onChange={e => setNonBillablePercent(parseFloat(e.target.value) || 0)}
                      className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white font-bold focus:border-blue-500/50 outline-none [color-scheme:dark]"
                    />
                  </div>
               </div>
            ...
            <div className="bg-blue-600 rounded-2xl p-10 text-center shadow-2xl shadow-blue-500/20 relative overflow-hidden group">
               ...
               <div className="relative z-10 space-y-8">
                  <div>
                    <div className="text-white/70 text-sm font-bold uppercase tracking-widest mb-2">Your minimum hourly rate</div>
                    <div className="text-6xl font-black text-white">${minimumRate.toFixed(2)}<span className="text-2xl font-medium opacity-60">/hr</span></div>
                  </div>
                  ...
                  <div className="inline-block px-8 py-6 bg-white border border-white rounded-2xl shadow-xl transition-transform group-hover:scale-105 duration-500">
                    <div className="text-blue-900/50 text-xs font-bold uppercase tracking-widest mb-1">Recommended Rate (with 20% buffer)</div>
                    <div className="text-4xl font-black text-blue-600">${recommendedRate.toFixed(2)}<span className="text-lg font-medium opacity-60">/hr</span></div>
                  </div>
        ...
        <div className="bg-[#111111] rounded-2xl p-10 md:p-16 border border-white/10 text-center mt-24 relative overflow-hidden shadow-2xl">
          ...
          <p className="text-ad-secondary text-lg mb-10 max-w-2xl mx-auto leading-relaxed">
            Once you know your rate — invoice at that rate professionally. AddInvoices makes it easy to create, send, and get paid on time.
          </p>
          <a
